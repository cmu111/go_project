...
func (p ProjectDao) FindProjectMemberByPid(ctx context.Context, projectId int64) ([]*data.ProjectMember, error) {
	var pms []*data.ProjectMember
	session := p.conn.Session(ctx)
	query := fmt.Sprintf("select * from ms_project_member where project_code=?")
	tx := session.Raw(query, projectId)
	err := tx.Scan(&pms).Error
	return pms, err
}
