...
func (t *TaskDao) FindTaskById(ctx context.Context, TaskCode int64) (ts *data.Task, err error) {
	err = t.conn.Session(ctx).Where("id=?", TaskCode).Find(&ts).Error
	return
}
...
func (t *TaskDao) FindTaskByAssignTo(ctx context.Context, id int64, done int, page int64, size int64) (tsList []*data.Task, total int64, err error) {
	session := t.conn.Session(ctx)
	index := (page - 1) * size
	err = session.Model(&data.Task{}).
		Where("assign_to=? and deleted =0 and done=?", id, done).
		Limit(int(size)).Offset(int(index)).Find(&tsList).Error
	err = session.Model(&data.Task{}).
		Where("assign_to=? and deleted =0 and done=?", id, done).
		Count(&total).Error
	return
...
func (t *TaskDao) FindTaskByCreatBy(ctx context.Context, id int64, done int, page int64, size int64) (tsList []*data.Task, total int64, err error) {
	session := t.conn.Session(ctx)
	index := (page - 1) * size
	err = session.Model(&data.Task{}).
		Where("create_by=? and deleted =0 and done=?", id, done).
		Limit(int(size)).Offset(int(index)).Find(&tsList).Error
	err = session.Model(&data.Task{}).
		Where("create_by=? and deleted =0 and done=?", id, done).
		Count(&total).Error
...
func (t *TaskDao) FindTaskMemberPage(ctx context.Context, taskCode int64, page int64, size int64) (tList []*data.TaskMember, total int64, err error) {
	session := t.conn.Session(ctx)
	offset := (page - 1) * size
	err = session.Model(&data.TaskMember{}).Where("task_code=?", taskCode).Limit(int(size)).Offset(int(offset)).Find(&tList).Error
	err = session.Model(&data.TaskMember{}).Where("task_code=?", taskCode).Count(&total).Error
	return
}

func (p *TaskDao) FindTaskByIds(ctx context.Context, tids []int64) (list []*data.Task, err error) {
	session := p.conn.Session(ctx)
	err = session.Model(&data.Task{}).Where("id in (?)", tids).Find(&list).Error
	return
}
