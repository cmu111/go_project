...
func (ps *ProjectService) GetLogBySelfProject(ctx context.Context, msg *project.ProjectRpcMessage) (*project.ProjectLogResponse, error) {
	logList, total, err := ps.projectLogRepo.FindLogByMemberCode(ctx, msg.MemberId, msg.Page, msg.PageSize)
	if err != nil {
		zap.L().Error("project GetLogBySelfProject FindLogByProjectCode error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	var taskIds []int64
	var mIds []int64
	...
	for _, v := range logList {
		taskIds = append(taskIds, v.SourceCode)
		mIds = append(mIds, v.MemberCode)
		pIdList = append(pIdList, v.ProjectCode)
	}
	var taskList []*data.Task
	var taskMap = make(map[int64]*data.Task)
	taskList, err = ps.taskRepo.FindTaskByIds(ctx, taskIds)
	for _, v := range taskList {
		taskMap[v.Id] = v
	}

	if err != nil {
		zap.L().Error("project GetLogBySelfProject FindTaskByIds error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	memberList, errm := rpc.LoginServiceClient.FindMemInfoByIds(ctx, &login.UserMessage{MIds: mIds})
	if errm != nil {
		zap.L().Error("project GetLogBySelfProject FindMemInfoByIds error", zap.Error(errm))
		return nil, errm
	}
	var memMap = make(map[int64]*login.MemberMessage)
	...
	var logsDisplay []*data.IndexProjectLogDisplay
	for _, v := range logList {
		logDisplay := v.ToIndexDisplay()
		logDisplay.MemberName = memMap[v.MemberCode].Name
		logDisplay.TaskName = taskMap[v.SourceCode].Name
		logDisplay.ProjectName = pMap[v.ProjectCode].Name
		logsDisplay = append(logsDisplay, logDisplay)
	}
	var logsMsg []*project.ProjectLogMessage
	copier.Copy(&logsMsg, logsDisplay)
	return &project.ProjectLogResponse{List: logsMsg, Total: total}, nil
}
