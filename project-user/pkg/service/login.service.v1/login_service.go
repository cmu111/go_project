...
func (ls *LoginService) Login(ctx context.Context, msg *login.LoginMessage) (*login.LoginResponse, error) {
	...
	token := jwts.CreateToken(memIdStr, msg.Ip, exp, config.C.JwtConfig.AccessSecret, rExp, config.C.JwtConfig.RefreshSecret)
	...
	go func() {
		...
		err = ls.cache.Put(c, model.Member+"::"+memIdStr, string(memJson), exp)
		orgJson, _ := json.Marshal(orgs)
		err = ls.cache.Put(c, model.MemberOrganization+"::"+memIdStr, string(orgJson), exp)
		if err != nil {
			zap.L().Error("Login cache put error", zap.Error(err))
		}
	}()
...
func (ls *LoginService) TokenVerify(ctx context.Context, msg *login.LoginMessage) (*login.LoginResponse, error) {
	...
	parseToken, ip, err := jwts.ParseToken(token, config.C.JwtConfig.AccessSecret)
	if err != nil {
